...
- The source package is a runtime dependency and its wrap script is installed by the `postinstall` hook on install and update, because the shipped skills shell out to the `openspec` CLI. See [ADR 0002](adr/0002-bundle-the-openspec-cli-behind-a-wrap-script.md).
